...
def name_clusters(vectorizer, kmeans, top_n_words=4, distinctiveness_weight=0.7):
    """
    Generate cluster names that balance distinctiveness and representativeness

    Parameters:
    - distinctiveness_weight: 0-1, how much to favor distinctive terms over common ones
                             (0.5 = equal weight, 0.7 = slightly favors distinctiveness)
    """
    feature_names = vectorizer.get_feature_names_out()
    cluster_centers = kmeans.cluster_centers_
    n_clusters = cluster_centers.shape[0]

    # Calculate two scores for each term:
    # 1. Representativeness: raw cluster center value (TF)
    # 2. Distinctiveness: inverse cluster frequency (ICF)

    # Calculate term presence across clusters
    term_presence = np.zeros_like(cluster_centers)
    for i in range(n_clusters):
        term_presence[i] = (cluster_centers[i] > np.percentile(cluster_centers[i], 75)).astype(int)
    term_presence_sum = term_presence.sum(axis=0)

    # Calculate ICF - more weight to terms that appear in fewer clusters
    ...
    # Common AI terms to deprioritize (add as needed)
    common_ai_terms = {
        'data', 'learning', 'model', 'network', 'neural',
        'algorithm', 'method', 'system', 'approach', 'based',
        'using', 'problem', 'result', 'performance', 'task',
        'analysis', 'information', 'process', 'feature', 'set'
    }

    # Minimum presence threshold (terms must appear in at least X% of cluster docs)
    min_presence = 0.15  # 15% of documents in cluster

    cluster_names = {}
    for i in range(n_clusters):
        # Get top terms based on combined scores
        ...
        for idx in top_indices:
            ...
            # Skip numeric, short, or stopword-like terms
            if (term.isdigit() or len(term) < 3 or
                    term in {'use', 'show', 'propose', 'present'}):
                continue

            # Skip common AI terms if we already have some good terms
            if (term in common_ai_terms and
                    len(top_terms) >= max(2, top_n_words // 2)):
                continue

            # Check if term has sufficient presence in cluster
            term_freq = cluster_centers[i][idx]
            max_freq_in_cluster = cluster_centers[i].max()
            if term_freq < min_presence * max_freq_in_cluster:
                continue
        ...
        # Fallback if we didn't get enough terms
        if len(top_terms) < top_n_words:
            # Take top terms by raw TF
            backup_indices = cluster_centers[i].argsort()[::-1]
            backup_terms = [feature_names[idx] for idx in backup_indices
                            if (not feature_names[idx].isdigit() and
                                len(feature_names[idx]) >= 3)]
            top_terms.extend(backup_terms[:top_n_words - len(top_terms)])
            top_terms = list(dict.fromkeys(top_terms))[:top_n_words]  # remove duplicates
